...
FROM base as build
COPY . /root/ws/src/«IF stack===null»«system.name.toLowerCase»«ELSE»«system.name.toLowerCase»_«stack.name.toLowerCase»«ENDIF»«IF imageInfo.get_ros_version()===2»_ros2«ENDIF»/
RUN --mount=type=bind,from=builder,target=/builder \
    apt-get update -qq && \
    «IF imageInfo.get_ros_version()===2»/builder/workspace.bash builder_setup && \«ENDIF»
