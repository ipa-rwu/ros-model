							...
							for (RosPublisher rospub : component.getRospublisher()) {
								for (ComponentInterface component2 : ((RosSystem) rossystem).getRosComponent()) {
									for (RosSubscriber rossub : component2.getRossubscriber()) {
										if (rospub.getPublisher().getMessage().getFullname().equals(rossub.getSubscriber()
												.getMessage().getFullname())
												&& rospub.getName().equals(rossub.getName())) {
											if (rospub.eContainer() != rossub.eContainer()) {
												duplicated = false;
												System.out.println("Topic Connection found [" + rospub.eContainer()
							...
							for (RosServiceClient rosscl : component.getRosserviceclient()) {
								for (ComponentInterface component2 : ((RosSystem) rossystem).getRosComponent()) {
									for (RosServiceServer rosss : component2.getRosserviceserver()) {
										if (rosscl.getSrvclient().getService().getFullname().equals(rosss.getSrvserver()
												.getService().getFullname())
												&& rosscl.getName().equals(rosss.getName())) {
											if (rosscl.getSrvclient().eContainer() != rosss.getSrvserver()
													.eContainer()) {
												duplicated = false;
												System.out.println("Service Connection found [" + rosscl.eContainer()
														+ "]" + rosscl.getName() + "->[" + rosss.eContainer() + "]"
														+ rosss.getName());
												// Check if connection already exists
							...
							for (RosActionClient rosac : component.getRosactionclient()) {
								for (ComponentInterface component2 : ((RosSystem) rossystem).getRosComponent()) {
									for (RosActionServer rosas : component2.getRosactionserver()) {
										if (rosac.getActclient().getAction().getFullname().equals(rosas.getActserver()
												.getAction().getFullname()) && rosac.getName().equals(rosas.getName())) {
											if (rosac.eContainer() != rosas.eContainer()) {
												duplicated = false;
												System.out.println("Action Connection found [" + rosac.eContainer()
