...
«"  "»«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»:
    image: "«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»:latest"
    depends_on:
      - ros-master
    environment:
      - "ROS_MASTER_URI=http://ros-master:11311"
      - "ROS_HOSTNAME=«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»"
    networks:
      - ros
...
«"  "»«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»_«stack.name.toLowerCase»:
    image: "«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»_«stack.name.toLowerCase»:latest"
    depends_on:
      - ros-master
    environment:
      - "ROS_MASTER_URI=http://ros-master:11311"
      - "ROS_HOSTNAME=«generator_helper.get_uniqe_name(system.name.toLowerCase, deploymentInfo.get_ros_distro())»_«stack.name.toLowerCase»"
    networks:
      - ros
