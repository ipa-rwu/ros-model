...
«"    "»spec:
      containers:
      - name: «pod_name»
        image: «dep_info.get_registry()»/«generator_helper.set_image_name(image_prefix, dep_info.get_ros_distro())»:«dep_info.get_image_version()»
        ...
        env:
          ...
          «IF rosVersion == 2»
          - name: ROS_DOMAIN_ID
            value: "«dep_info.get_rosDomainID()»"
          «ENDIF»
        «IF if_connect_web»
        ports:
...
«create_container(generator_helper.set_pod_name(system.name),
				...
				system.name,
				deploymentInfo,
				generator_helper.set_start_command(system.name, deploymentInfo.get_ros_version),
...
«create_container(generator_helper.set_pod_name(system.name, stack.name),
				...
				deploymentInfo,
				generator_helper.set_start_command(system.name, stack.name, deploymentInfo.get_ros_version),
				deploymentInfo.get_ros_version(),
				k8s_info.get_ifConnectWeb(),
				k8s_info.get_ifUseMacvlan()
)»
