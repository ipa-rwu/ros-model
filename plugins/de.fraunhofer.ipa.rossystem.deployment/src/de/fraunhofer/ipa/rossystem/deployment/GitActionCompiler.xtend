...
name: «generator_helper.get_uniqe_name(system.name.toLowerCase, imageInfo.get_ros_distro().toString)»
on:
  push:
    paths:
      - '«generator_helper.set_system_folder_name(system.name.toLowerCase, imageInfo.get_ros_distro().toString)»/**'
env:
...
jobs:
  ...
  «extra_layer(system.name.toLowerCase, imageInfo.get_ros_distro().toString,imageInfo.get_image_version())»
  «system_layer(system.name.toLowerCase, true, imageInfo.get_ros_distro().toString, imageInfo.get_image_version())»
	«ELSE»
  «system_layer(system.name.toLowerCase, false, imageInfo.get_ros_distro().toString, imageInfo.get_image_version())»
«ENDIF»
«ELSE»«FOR stack : system.getComponentStack()»«IF !generator_helper.listOfRepos(stack).isEmpty()»
  «extra_layer(system.name.toLowerCase, stack.name.toLowerCase, imageInfo.get_ros_distro().toString, imageInfo.get_image_version())»
  «stack_layer(system.name.toLowerCase, stack.name.toLowerCase, imageInfo.get_ros_distro().toString, imageInfo.get_image_version(), true)»
 «ELSE»
  «stack_layer(system.name.toLowerCase, stack.name.toLowerCase, imageInfo.get_ros_distro().toString, imageInfo.get_image_version(), false)»
«ENDIF»
 «ENDFOR»
«ENDIF»
'''
